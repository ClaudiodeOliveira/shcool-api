...
public class ApiExceptionHandler {
    ...
    private static final String APPLICATION_NAME = "Scholl";
    ...
    public ResponseEntity<ApiErrorResponse> constraintValidacaoHandler(ConstraintViolationException exception) {
        ApiErrorResponse apiErrorResponse = ApiErrorResponse.builder()
                .id(UUID.nameUUIDFromBytes(APPLICATION_NAME.getBytes(StandardCharsets.UTF_8)).toString())
                .titulo(ERROR_BAD_REQUEST)
                .codigo(String.valueOf(HttpStatus.BAD_REQUEST.value()))
                .status(HttpStatus.BAD_REQUEST.name())
                .detalhe(exception.getMessage())
                .metadado(
                        ApiErrorResponse.Metadado.builder()
                                .nmServico(APPLICATION_NAME)
                                .idTransacao(UUID.nameUUIDFromBytes(APPLICATION_NAME.concat(LocalDateTime.now().toString()).getBytes(StandardCharsets.UTF_8)).toString())
